...
public class FlutterJNI {
  ...
  public void attachToNative(boolean isBackgroundView) {
    ...
    // AndroidShellHodler在Java层的指针,long指针
    nativePlatformViewId = nativeAttach(this, isBackgroundView);
  }


  // attachToNative方法中通过调用JNI方法nativeAttach将当前flutterJNI对象传递给c++层
  //（后续一些dart层调用java层的方法就是通过该对象调用对应的方法实现的），
  // 得到c++层返回的nativePlatformViewId,这个值非常重要，
  // 是c++层AndroidShellHolder的对象指针值，后续会通过该值调用一系列c++层的方法执行操作,
  // 并将其保存以供后续使用。
  // shell/platform/android/platform_view_android_jni.cc 中
  // 
